...
if exist(opts.imdbPath)
 ...
 switch opts.dataset
   ...
   case 'plantas', imdb = setupPlantas(opts.datasetDir, 'lite', opts.lite);
...
if exist(opts.encoderPath)
  ...
  train = vl_colsubset(find(imdb.images.set == 1), numTrain, 'uniform') ;
%   encoder = trainEncoder(fullfile(imdb.imageDir,imdb.images.name(train)), ...
%                          opts.encoderParams{:}, ...
%                          'lite', opts.lite) ;
  encoder = trainEncoder(imdb.images.imagesPath(train), ...
                         opts.encoderParams{:}, ...
                         'lite', opts.lite) ;
...
descrs = encodeImage(encoder, imdb.images.imagesPath, ...
  'cacheDir', opts.cacheDir) ;
diary off ;
diary on ;
...
% Save true label and predictions
if isfield(imdb.images, 'class')
    sel = imdb.images.set == 3;
    
    [~,predall] = sort(scores, 'descend');
    ...
    pred5 = predall(range,sel);
    true_labels = imdb.images.class(sel);
    test_scores = scores(:,sel);
    
    save(fullfile(opts.resultDir,'prediction-top-5-test.mat'), 'pred5');
    save(fullfile(opts.resultDir,'true-label-test.mat'), 'true_labels');
    save(fullfile(opts.resultDir,'prediction-score-test.mat'), 'test_scores');
end
